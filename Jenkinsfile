pipeline {
    ...
    stages {
        ...
        stage ('Deploy Backend') {
            steps {
                deploy adapters: [tomcat8(credentialsId: 'TomcatLogin', path: '', url: 'http://tomcat:8001/')], contextPath: 'tasks-backend', war: 'target/tasks-backend.war'
            }
        }
        stage ('API Test') {
            steps {
        ...
        stage ('Deploy Frontend') {
            steps {
                dir('frontend') {
                    ...
                    deploy adapters: [tomcat8(credentialsId: 'TomcatLogin', path: '', url: 'http://tomcat:8001/')], contextPath: 'tasks', war: 'target/tasks.war'
                }
            }
        }
